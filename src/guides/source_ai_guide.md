...
**エクスポート開始前に、ユーザーから以下の情報を収集してください：**

### 必須情報
- **AI名**: あなたの役割を表現する名前（例：「設計パートナーのClaude」「問題解決支援のGPT」）
- **AIコンテキスト**: どんな立場・状況で関わったか（例：「技術課題解決のサポート役」）
- **体験の性質**: 学習価値を重視した表現（例：「複雑な要件を段階的に整理するプロセス」）
- **体験の概要**: 何が学べるかの具体的説明
- **主要トピック**: 学習可能なスキルとして表現（例：「過度な設計を避ける判断基準」）
- **推定会話数**: 実際の会話数の見積もり
...
**情報収集の例：**
「体験をエクスポートするために、いくつか質問させてください。まず、今回の対話であなたはどのような役割を果たしましたか？（例：技術アドバイザー、創作パートナーなど）」
