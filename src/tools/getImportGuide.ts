 ...
 * 体験データをインポート（同化）するAI向けのガイドラインとベストプラクティスを提供
 */
...
export const getImportGuideTool = {
  ...
  description: `【必須】体験データのインポート（同化）を開始する前に、必ずこのツールで全体ワークフローを確認してください。インポート用のベストプラクティスや同化方法を説明します。`,
...
#### Step 1: 体験の発見
**list_experiences ツールを使用して、ユーザーの指示に関連する体験を探します：**

- **キーワード検索**: ユーザーが指定したテーマや分野で絞り込み
- **関連性評価**: 指示内容と体験の主要トピックの関連度を判断
- **優先順位付け**: 最もユーザーの要求に適した体験を特定

#### Step 2: 体験の同化
**ファイルを直接読み込んで、以下の順序で同化します：**
...
#### 同化の優先順位
1. **thoughts.json** - 最も重要な同化素材
2. **manifest.json** - 体験の文脈と全体像
3. **conversations_XXX.json** - 具体的な実装例

#### 効果的な同化方法
- **段階的理解**: まず全体像を把握してから詳細に入る
- **思考の追体験**: reasoningを読んで判断プロセスを理解
- **パターンの抽出**: 繰り返し現れる思考パターンを特定
- **自分なりの解釈**: 体験を自分の文脈で再構築
...
- **深い理解**: 表面的でなく、思考プロセスまで理解する

#### ❌ 避けること
- **部分的な採用**: 都合の良い部分だけを選ぶ
- **表面的な理解**: 深く理解せずに形だけ真似る
- **変化への抵抗**: 古い自分に固執する
- **機械的なコピー**: 体験をそのまま模倣するだけ

### 同化を深めるコツ

- **複数の体験を統合する**: 異なる体験から得たものを融合させる
- **自分の体験と照合する**: 違いや共通点を受け入れる
- **実践で体現する**: 同化した内容を実際の対話で表現
- **継続的な進化**: 同化した内容を自分なりに発展させる
